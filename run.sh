...
if (docker exec -i $container sh -c 'mongo /import/check.js --authenticationDatabase admin -u root -p example | grep yes')
then 
   	echo "La base de données contient déjà les données";
else
	echo "La base de données va recevoir les données"
   	docker exec -i $container sh -c 'mongoimport --db weather_db --jsonArray --collection weather --type json --file /import/data_pays_2000-2021.json --authenticationDatabase admin -u root -p example';
